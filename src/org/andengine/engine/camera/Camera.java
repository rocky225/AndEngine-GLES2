...
/**
 * A Camera defines the rectangle of the scene that is drawn on the screen, as
 * not the whole scene is visible all the time. Usually there is one Camera per
 * Scene, except for the SplitScreenEngines. There are subclasses that allow
 * zooming and smooth position changes of the Camera. <br>
 * 一个Camera会确定一个画在屏上的矩形的场景，由于整个场景并不是一直可见。通常每个Secene一个Camera，除非splitscreenengines
 * 。它允许放大，平滑的变换位置。<br>
 * <br>
